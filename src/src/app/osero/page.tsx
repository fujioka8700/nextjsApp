...
const OthelloPage: React.FC = () => {
  ...
  const getValidMoves = useCallback(
    (player: Player, currentBoard: BoardState): number[][] => {
      const moves: number[][] = [];
  ...
  const initGame = useCallback(() => {
    ...
    initialBoard[3][3] = COMPUTER;
    initialBoard[3][4] = PLAYER;
    initialBoard[4][3] = PLAYER;
    initialBoard[4][4] = COMPUTER;
  ...
  const computerMove = useCallback(() => {
    ...
    if (difficulty === 'easy') {
      ...
      bestMove = validMoves.reduce(
        ...
        { move: validMoves[0], score: -Infinity },
      ).move;
    }
    ...
    <div className={styles.container}>
      <div className={styles.gameContainer}>
        ...
        <div className={styles.gameBoard}>
          {board.map((row, r) =>
            row.map((cell, c) => {
              const isMovable = validMoves.some(
                (m) => m[0] === r && m[1] === c,
              );
              return (
                <div
                  ...
                  className={`${styles.cell} ${
                    isMovable ? styles.validMove : ''
                  }`}
                  onClick={() => isMovable && handlePlayerMove(r, c)}
                >
                  {cell && (
                    <div className={`${styles.disc} ${styles[cell]}`}></div>
                  )}
                </div>
              );
            }),
